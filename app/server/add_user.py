...
class RegisterUser(QDialog):
    """
    Класс предназначен для регистрации пользователя на сервере
    """
    ...
    def save_data(self):
        ...
        if not self.client_name.text():
            self.messages.critical(self, 'Имя пользователя не указано')
            return
        elif self.client_passwd.text() != self.client_conf.text():
            self.messages.critical(self, "Введенные пароли не совпадают")
            return
            ...
            password_hash = hashlib.pbkdf2_hmac('ololo2007', password_bytes, salt, 10000)
            self.database.add_user(self.client_name.text(), binascii.hexlify(password_hash))
            self.messages.information(self, 'Успешно!', "Пользователь зарегистрированю")
